...
\semblend achieves $\mathbf{2.2\times}$--$\mathbf{12.0\times}$ TTFT
...
\item \textbf{Empirical evaluation} on two model architectures
  ...
  datasets, demonstrating $2.2\times$--$\mathbf{12.0\times}$ TTFT speedup at
...
at 8K tokens, speedup ranges 1.5--3.8$\times$ (Tables~\ref{tab:cross-dataset}--\ref{tab:multi-model-cross}).
...
$^{**}$After sliding window text sampling: 87.5\% hit rate at 8K
(was 0\% with head-only truncation; see \S\ref{sec:design:embedding}).
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Instruction Variant} & \textbf{$\Delta$ chars} & \textbf{LMCache} & \textbf{SemBl.} & \textbf{Gap} \\
\midrule
Donor (``You are a helpful assistant...'') & ---  & \multicolumn{3}{c}{(baseline: $3{,}420$\,ms cold)} \\
\midrule
``You are an expert summarizer...'' & $-9$  & 0\%  & 100\%  & $3.31\times$ \\
``You assist users by summarizing...'' & $-12$ & 0\%  & 100\%  & $3.44\times$ \\
``Summarize documents accurately...''  & $-20$ & 0\%  & 100\%  & $3.43\times$ \\
``You provide accurate summaries.''    & $-25$ & 0\%  & 100\%  & $3.38\times$ \\
\midrule
\textbf{Mean} & & \textbf{0\%} & \textbf{100\%} & $\mathbf{3.39\times}$ \\
\bottomrule
\end{tabular}\\[2pt]
{\scriptsize LMCache Hit = fraction of 256-token chunk hashes matching donor.
Zero because $|\Delta\text{chars}|{\geq}9$ shifts all chunk boundaries.
SemBlend Hit = fraction of queries routed to donor via cosine sim ${\geq}0.60$.
Speedup vs.\ cold prefill (LMCache-only speedup $=1.0\times$ for all rows).
100\% hit rate sustained at 16K tokens ($3.16\times$--$3.30\times$ speedup).}
\end{table}
...
$1.5\times$--$3.8\times$ at 8K tokens (Qwen: $1.97$--$2.37\times$;
LLaMA: $2.66$--$3.78\times$ hit-only). SemBlend TTFT remains
...
speedup ranges $1.5\times$--$3.8\times$ at 8K tokens.
