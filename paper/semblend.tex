...
4 KV heads). Table~\ref{tab:multi-model-cross} shows results on XSum
and CNN/DailyMail with identical cluster construction methodology.
...
\midrule
\multirow{6}{*}{\shortstack[l]{Qwen2.5-7B\\(28L, 4 KV)}}
...
\midrule
\multirow{6}{*}{\shortstack[l]{LLaMA-3.1-8B\\(32L, 8 KV)}}
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize LLaMA-3.1-8B achieves substantially higher hit-only speedups
($2.66$--$3.34\times$) than Qwen ($1.51$--$2.29\times$) across both datasets,
likely due to its deeper architecture (32 vs 28 layers) increasing cold prefill
cost while SemBlend's KV transfer overhead remains constant.}
\end{table}
...
$1.5$--$2\times$ higher. This is because LLaMA's 32-layer architecture has
...
means deeper models see proportionally larger speedups.

\paragraph{Quality preservation across architectures.}
LLaMA quality on XSum shows PPL ratios of 1.009 (2K), 1.011 (4K), and
1.065 (8K)---comparable to Qwen's quality results and well within the
$< 1.05$ threshold for negligible degradation at practical lengths.
...
with zero errors at all levels. P50 TTFT increases from ${\sim}340$\,ms
($c{=}1$) to ${\sim}1000$\,ms ($c{=}32$), matching expected vLLM behavior
...
\paragraph{Ablation: PartialAttention contribution.}
To isolate the contribution of bathtub recomputation
(\S\ref{sec:recomp}), we disable PartialAttention and measure TTFT
and quality with KV injection alone. Without recomputation, synthetic
8K speedup is $5.51\times$ (vs.\ $5.66\times$ with recomputation)---a
marginal 2.6\% difference---while PPL ratios remain acceptable
(0.964--1.018). This confirms that the primary speedup derives from
KV transfer bypassing full prefill, not from recomputation savings.
PartialAttention's value is thus primarily in \emph{quality insurance}
for high-drift layers, not TTFT improvement, and can be disabled for
workloads prioritizing maximum throughput over strict quality parity.

\paragraph{Threats to validity.}
(1)~KV-tensor speedups are measured with streaming TTFT on a
single-request workload; throughput measurements at $c{=}1$--$32$
confirm zero degradation (9+ QPS, zero errors) but use short prompts.
...
Six findings stand out: (1)~semantic KV donor injection achieves
...
(2)~on real-world NLP datasets (CNN/DailyMail, XSum, MultiNews),
SemBlend achieves $1.5\times$--$3.3\times$ TTFT speedup with
100\% hit rates on XSum at 8K tokens, and LLaMA-3.1-8B consistently
achieves higher hit-only speedups ($2.7\times$--$3.3\times$) than
Qwen ($1.5\times$--$2.3\times$) due to its deeper architecture
amplifying the constant-overhead advantage;
...
(4)~embedding-based donor discovery supersedes Jaccard for identifying
...
$<$12\,ms pipeline overhead does not limit serving capacity;
(6)~SemBlend TTFT is approximately constant (${\sim}650$--$910$\,ms)
