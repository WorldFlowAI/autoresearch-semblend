...
architectures, \semblend achieves $2.2$--$\mathbf{8.3\times}$ TTFT speedup
on same-context reuse (2K--16K tokens, 100\% hit rate on Qwen; 60\% at 16K on
LLaMA due to larger KV footprint) and $1.5$--$3.0\times$ mean on mixed
...
& 16K tokens & 6,604 &  794 & $\mathbf{8.32\times}$ & 100\% & 5 \\
\midrule
...
& 16K tokens & 6,377 &  824 & $\mathbf{7.74\times}^*$ &  60\% & 5 \\
\bottomrule
\end{tabular}\\[2pt]
...
$^*$Hit-only speedup; LLaMA 16K hit rate limited by LMCache CPU buffer
capacity (8 KV heads $\times$ 32 layers = $2\times$ memory vs Qwen's
4 KV heads $\times$ 28 layers). On real NLP datasets at 8K tokens,
speedup ranges 1.5--3.8$\times$ (Table~\ref{tab:cross-dataset}).
Peak individual: $9.65\times$ (Qwen 16K).}
\end{table*}
