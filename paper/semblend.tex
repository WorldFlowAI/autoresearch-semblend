...
The paper further identifies a \emph{safety cliff} at ${\approx}90\%$
token compression where hallucination rates spike nonlinearly---caused
by simultaneous eviction of answer-critical tokens from all attention
heads. \semblend operates in an entirely different regime (Global
Eviction Ratio $= 0$: no tokens are evicted from matched blocks),
so the cliff is irrelevant to our approach.
...
On an 8-worker fleet (LLaMA-3.1-8B-AWQ-INT4), 100\% of queries route
to a semantically matching worker, achieving $2.6\times$ TTFT speedup
on exact-match pairs and $3.8\times$ at 16K tokens
...
attending over the merged donor+fresh KV cache. The three kernels are
chained in a single CUDA stream with a PyTorch CPU fallback, and
validated by 16 unit tests covering plan construction, attention mask
computation, and per-layer reuse decisions.
...
Hit rate \emph{never degrades} as the pool grows---each additional
donor adds coverage without harming existing matches.
At 2K tokens, no LRU eviction occurs (each 2K-token donor requires
${\sim}112$\,MB, leaving room for 100+ donors in the 15\,GB CPU buffer)
and speedup is constant across the full $N{=}1$--100 range.
...
We benchmark donor store search latency across five scales on a
dedicated A10G node (22.8\,GB free), measuring \textbf{numpy cosine
...
An early deployment revealed an \textit{EOS-collapse} artifact
(${\approx}25\%$ of exact-match hits) in which the injected donor KV
placed the attention state at a natural sentence boundary, causing the
model to emit \texttt{</s>} immediately. Root cause: \texttt{max\_usable
= prompt\_len - 1} left only one fresh token for the model to process.
Fix: \texttt{min\_fresh\_tokens = 256} (one LMCache chunk) guarantees
the model processes sufficient real tail context before generating.
Post-fix validation ($n{=}16$) shows $\mathbf{0\%}$ collapse rate with
mean completion of 200 tokens (see Section~\ref{sec:limitations}).
For non-collapsing runs: at 512 tokens, PPL ratio $= 1.048$ and
ROUGE-L ${\approx}0.85$--$1.0$; at 1024 tokens, PPL ratio $= 0.997$
(better than cold) and ROUGE-L $= 0.677$. The ROUGE-L decrease from
512 to 1024 tokens is expected---early-token divergence cascades over
long generation---while PPL remains stable near 1.0, confirming
SemBlend quality at production generation lengths.
...
Qwen2.5-7B-AWQ deployment (28 layers, A10G, autoresearch namespace)
...
The break-even hit rate for SemBlend to be beneficial
is $P_h > 0$ (there is no overhead to overcome), since
TTFT$_\text{hit} \ll$ TTFT$_\text{cold}$ at any length
and the 8\,ms pipeline overhead is negligible.

\paragraph{Temperature invariance.}
\semblend's KV injection is a prefill-side operation; output sampling
temperature only affects decode. We verify this empirically on the
cross-instruction RAG workload (XSum 8K, $n{=}8$,
same-document different-instruction queries):
temperatures $T \in \{0.3, 0.7, 1.0\}$ all achieve 100\% hit rate
and indistinguishable TTFT ($1.87\times$--$1.90\times$ speedup,
$\pm 1.5\%$ variance across temperatures).
This confirms \semblend is production-safe at any sampling temperature.
...
identity does not guarantee semantic position alignment. Per-chunk
quality scoring (ProphetKV-style selective recomputation~\cite{prophetkv})
would filter such cases at the cost of additional overhead.

\paragraph{Entity-swap FM2 validation.}
To directly probe whether injected KV causes hallucination from stale
entity context, we conducted a targeted entity-swap experiment: for
8 CNN/DailyMail 8K clusters, we swapped 1--3 named entities in the
query (e.g., person names, organizations, locations) while using the
original article as donor. Under this adversarial perturbation,
PPL ratio $= 1.000$ at all swap counts (1-swap: hit rate 12.5\%;
2-swap: hit rate 0\%; 3-swap: hit rate 12.5\%). On hits, the system
produces correct output relative to the swapped query---not the donor's
original entities. This follows from LMCache's block-hash gate:
entity swaps change token sequences, blocking block-level matches and
causing the system to fall back to cold prefill, which has PPL$=1.0$
by definition. SemBlend \emph{does not hallucinate stale donor entities}.
...
we confirm mathematically that NoPE and delta correction are equivalent (RoPE group property: $\text{RoPE}(a)\text{RoPE}(-b)=\text{RoPE}(a-b)$). The experiment confirms
...
A runtime ablation setting \texttt{SEMBLEND\_USE\_ALIGNMENT=0} on the
live deployment confirms $\Delta{=}0$ empirically: PPL ratio
$=1.000{\pm}0.006$ across all 7 variation types ($n{=}8$, XSum 8K),
identical to the full-correction baseline, confirming that RoPE
correction is a no-op for all matched chunk-boundary blocks in the
current deployment.
The runtime overhead is negligible (${\sim}7\,\mu$s/layer), so
...
recalibration.
\textbf{Architecture-specific layer criticality}: Recent work
characterizes architecture-specific KV layer importance: LLaMA-family
transformers exhibit an ``inverted funnel'' pattern (early layers
consolidate information; middle/late layers are more redundant),
while Qwen-family exhibits a ``funnel'' pattern (late layers consolidate;
early layers more redundant)~\cite{kvphysics}. Our architecture-specific bathtub presets implement this directly:
LLaMA recomputes early layers $[0,1,2]$ (9.4\%)~---aligned with its
inverted-funnel pattern---while Qwen recomputes late layers $[26,27]$
(7.1\%)---aligned with its funnel pattern. Both are close to the 11\%
critical layer estimate from~\cite{droidspeak}. Our measured PPL ratios
(1.001--1.065 across five datasets and two models) confirm this
architecture-aware calibration is near-optimal in practice.
Cross-instance donor discovery via the optional CAGRA tier is validated on
an 8-worker fleet (\S\ref{sec:scaled}), achieving 100\% semantic
routing and $2.4$--$3.8\times$ speedup. Isolation testing at the
instance level is confounded by vLLM's internal RadixAttention
prefix cache, which caches previously-seen prompts within each instance;
empirical cross-instance isolation requires non-identical prompts and
a controlled gateway routing setup.
Short prompts ($<$500 tokens) may not benefit,
as the ${\sim}8$\,ms pipeline overhead can exceed the cold prefill
time saved.
\textbf{EOS-collapse (fixed)}\label{sec:limitations:eos}: During
development, ${\approx}25\%$ of exact-match 8K hits at
\texttt{max\_tokens=512}/\texttt{1024} triggered immediate
\texttt{</s>} emission. Root cause: \texttt{max\_usable = prompt\_len -
1} allowed the donor to fill all but one token of the prompt, leaving
the model with no real tail context. The attention state ``saw'' a
completed sentence boundary and immediately generated EOS. Fix
(\texttt{min\_fresh\_tokens = chunk\_size = 256}): \texttt{max\_usable
= max(chunk\_size, prompt\_len - chunk\_size)} reserves at least one
full 256-token LMCache chunk of fresh context. Post-fix empirical
validation ($n{=}16$, XSum 8K exact-match hits) shows $\mathbf{0\%}$
collapse rate, mean completion 200 tokens. This fix is deployed in the
production connector and all results in this paper were collected after
its deployment. Non-collapsing hit quality is unchanged: PPL ratio
$= 1.048$ at 512 tokens, $\mathbf{0.997}$ at 1024 tokens. ROUGE-L
decreases from ${\approx}0.85$--$1.0$ (512 tok) to 0.677 (1024 tok)
due to cascading text divergence---expected for long generation---while
PPL confirms true quality.
...
not KV injection. Across all 7 variation types from cosine 1.0 to 0.20,
PPL ratio is $1.000{\pm}0.001$ on both architectures---SemBlend provides
a \emph{quality floor guarantee}: hits use verified exact-match KV blocks,
misses fall back to cold inference (PPL $\equiv 1.0$).
