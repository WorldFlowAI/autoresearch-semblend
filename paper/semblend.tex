...
calibrated bathtub curve (\S\ref{sec:recomp}) implements a similar
principle: recomputing layers $[0,1,31]$ (9.4\%) for LLaMA and
$[0,1,2,27]$ (14.3\%) for Qwen. Architecture-specific tuning
(shifting Qwen recomputation toward late layers) is identified as
future work.
...
early layers more redundant)~\cite{kvphysics}. Our current bathtub
preset recomputes layers $[0,1,31]$ (9.4\%) for LLaMA and $[0,1,2,27]$
(14.3\%) for Qwen---close to the 11\% critical layer estimate from
selective recomputation studies~\cite{droidspeak}. Architecture-specific calibration
(shifting Qwen recomputation toward late layers, LLaMA toward early layers)
is a tractable future improvement. Our measured PPL ratios (1.001--1.065
across five datasets and two models) indicate the current uniform calibration
is already near-optimal in practice.
