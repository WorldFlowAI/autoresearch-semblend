...
\textbf{(2)~RoPE delta correction:}
Disabling RoPE correction shows no quality degradation (PPL
0.994--1.000, ROUGE-L 0.73--1.00). This is expected: LMCache's
256-token chunk hashing reuses chunks at \emph{matching positions},
making the RoPE delta zero. RoPE correction becomes essential when
chunks are reused at non-matching positions---which occurs with
sub-chunk alignment (future work) or alternative KV backends
supporting finer-grained reuse. The correction is mathematically
exact and computationally negligible (${\sim}7\,\mu$s/layer),
so it is enabled by default as forward-compatible insurance.
