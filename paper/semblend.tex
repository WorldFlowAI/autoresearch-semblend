...
with zero errors at all levels. P50 TTFT at $c{=}1$ is ${\sim}340$\,ms; P99 at $c{=}32$
reaches ${\sim}950$--$1037$\,ms, matching expected vLLM behavior
...
(3)~SemBlend Local uses MiniLM-L6-v2 (384-dim, 512-token context
window) which has lower embedding quality than jina-v4 (1024-dim,
longer context). For prompts where semantic content is distributed
beyond 512 tokens, MiniLM sees only the document beginning---an
ablation showed 7\% donor match rate vs.\ 99\% for full-token Jaccard
on 8K-token cluster prompts. In practice, the 512-token window
suffices for detecting similarity between variations of the same source
article (62--100\% cross-dataset hit rates), the dominant RAG pattern.
