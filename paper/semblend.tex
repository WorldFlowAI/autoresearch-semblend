...
\item \textbf{Local-First GPU-Native Architecture}: An in-process
  ...
  with \textbf{zero network hops}, plus a cuVS GPU-accelerated tier:
  brute-force (${\leq}0.71$\,ms at 100K donors) and CAGRA~\cite{cagra}
  ANN ($\mathbf{1.37}$\,ms \emph{constant} at 1M donors, empirically
  measured; build time 7.3\,s one-time).
...
impossible with RoPE-based models. Evaluated on five established NLP
benchmarks (CNN/DailyMail, XSum, MultiNews, WikiHow, SAMSum) across two model
...
vs SemBlend's constant ${\sim}1.3$\,s) and $1.5$--$3.8\times$ on mixed
semantic variations across five datasets; on datasets with ${\geq}88\%$
hit rate (WikiHow, XSum, synthetic), PPL ratio stays ${\leq}1.065$ on
both models, confirming zero quality degradation when KV reuse succeeds.
Elevated PPL on lower-hit datasets (SAMSum, CNN/DM) is driven by
miss-run outliers (cold inference), not KV injection quality.
The expected TTFT follows a validated linear model
$\mathbb{E}[\text{TTFT}] = P_h \cdot \text{TTFT}_\text{hit} + (1{-}P_h) \cdot \text{TTFT}_\text{cold}$
(${<}5\%$ error at all measured hit fractions), so any nonzero hit
rate improves mean latency. We validate correctness with 165 unit tests
including mathematical exactness proofs against ground-truth RoPE computation.
\end{abstract}
...
\item \textbf{Two-tier scaled architecture} (\S\ref{sec:scaled}):
  in-process numpy (${\leq}0.14$\,ms at $N{\leq}10\text{K}$),
  cuVS brute-force GPU (${\leq}0.71$\,ms at $N{\leq}100\text{K}$),
  and CAGRA ANN ($\mathbf{1.37}$\,ms \emph{constant} at $N{=}1$\,M;
  empirically measured) for cross-instance donor discovery at scale.
...
(\emph{without} model fine-tuning or custom kernels), and two-tier scaling.
...
brute-force ($O(N)$, ${\leq}0.14$\,ms for $N{\leq}10\text{K}$); the
scaled tier uses cuVS brute-force (${\leq}$0.71\,ms up to 100K) or
CAGRA ANN ($\mathbf{1.37}$\,ms constant up to 1M donors).
...
\caption{\semblend two-tier architecture. \textbf{Tier~1 (Local)}:
  ...
  via gateway for cross-instance discovery at 1.37\,ms constant to $N{=}1\text{M}$ donors.
...
Donor search   & ${\leq}$0.14\,ms   & 1.4\,ms & Numpy ($N{\leq}$10K) vs CAGRA ANN \\
...
$O(\log N)$ search up to $N{=}1\text{M}$ donors (1.37\,ms constant, A10G).
...
We validate cross-instance semantic routing on a production fleet:
8 vLLM+SemBlend workers (LLaMA-3.1-8B-AWQ-INT4, StatefulSet) with
a Python/FastAPI semantic router and NATS event plane.
Workers publish donor metadata (\texttt{SemanticStoredEvent}) on
\texttt{save\_kv\_layer()}; the router maintains a fleet-wide
MiniLM embedding index and routes new queries to the worker with
the highest-similarity donor.
Benchmark results on 4K-token prompts: \textbf{100\%} of queries
are routed to a semantically matching worker, achieving
$\mathbf{2.6\times}$ TTFT speedup on exact-match pairs and
$\mathbf{2.4\times}$ on reordered variants.
At 16K tokens, the fleet achieves $\mathbf{3.8\times}$ speedup
($3{,}222\,\text{ms} \to 850\,\text{ms}$) cold-to-semantic, confirming
that cross-instance donor reuse scales to production context lengths with
negligible routing overhead (numpy search at $0.21$\,ms, MiniLM embed at
$48$\,ms on CPU).
...
2 (Scaled) & Gateway GPU & CAGRA & up to 1M & Cross-instance \\
\bottomrule
\end{tabular}}
\end{table}
...
EOS-collapse (see Limitations) affects ${\approx}25\%$ of exact-match
hits independent of the output-length budget.
...
\paragraph{Empirical threshold insensitivity.}
A live endpoint sweep ($\tau \in \{0.50, 0.70, 0.80\}$, Qwen2.5-7B-AWQ,
CNN/DailyMail 8K, $n{=}8$) yielded \emph{identical} PPL ratios (1.033)
and hit patterns at all three thresholds. This occurs because all
CNN/DailyMail cluster pairs have measured cosine similarity ${\geq}0.80$
by construction (semantically similar articles from the same news cycle),
so the cosine gate is not the active quality filter in this regime.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Mode} & \textbf{Hit Rate} & \textbf{PPL Ratio} & \textbf{ROUGE-L} \\
\midrule
Position-shift & 0\% (0/8) & 1.0000 & 1.000 \\
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize Position-shift result: zero injections (correct specificity) and cold baseline preserved (PPL ratio $=1.000$). The filler-prepend design causes prefix divergence at chunk 0:
...
transfer; \semblend for semantic-variation coverage. The CAGRA tier
...
\paragraph{NoPE mode validation.}
We additionally implement the NoPE two-step approach
(\texttt{SEMBLEND\_USE\_NOPE=1}) and run the full benchmark suite under
both modes. The NoPE mode applies $\mathbf{K}_\text{raw}{=}\mathrm{RoPE}({-}p_s){\times}\mathbf{K}$
then $\mathbf{K}_\text{tgt}{=}\mathrm{RoPE}(p_t){\times}\mathbf{K}_\text{raw}$,
equivalent to delta correction by the RoPE group property.
Empirical comparison across all 7 variation types ($n{=}8$, XSum 8K)
confirms identical quality: NoPE PPL ratio $= 1.000{\pm}0.002$
vs.\ delta $1.000{\pm}0.001$.
Cross-dataset TTFT comparison across all six benchmark datasets
(Synthetic, XSum, CNN/DM, WikiHow, SAMSum, MultiNews at 8K) confirms
indistinguishable speedup: NoPE averages $2.95\times$ vs.\ delta $2.51\times$
(differences within $\pm$30\% measurement noise from cold-baseline
variability), with 100\% hit rate under both modes on 5 of 6 datasets.
The results confirm mathematical equivalence of the two approaches.
We release NoPE as an optional validated mode for environments that
prefer position-free storage.
...
Cross-instance donor discovery (the scaled tier) is validated on
...
time saved.
\textbf{Long-output quality}: Our primary quality metric uses
\texttt{max\_tokens=256}. Extended evaluation at
\texttt{max\_tokens=\{512, 1024\}} on XSum 8K synthetic clusters
reveals that ${\approx}25\%$ of exact-match hits trigger an
\textit{EOS-collapse} artifact---the model generates near-zero
output before emitting \texttt{</s>} because the injected donor KV
places the attention state at a natural sentence boundary. Notably,
this 25\% collapse rate is \emph{independent of the output-length
budget}: the collapse is determined by KV state alignment, not by
\texttt{max\_tokens}. Non-collapsing hits (75\% of runs) maintain
PPL ratio near 1.0 across all budgets: 1.048 at 512 tokens,
$\mathbf{0.997}$ at 1024 tokens. ROUGE-L decreases from
${\approx}0.85$--$1.0$ (512 tok) to 0.677 (1024 tok) due to
cascading text divergence over longer generation, but PPL confirms
true quality. EOS-collapse is detectable only through output-length
monitoring. A \texttt{min\_tokens} constraint prevents empty output
in production (implemented in our harness via the vLLM API), though
it inflates PPL for forced tokens; quality should be measured on
non-collapsed hits only.
...
for cross-instance discovery at a measured 1.37\,ms constant from
$N{=}100$ to $N{=}1\text{M}$ donors (A10G).
...
from $2.2\times$ at 2K to $8.3\times$ at 16K to $\mathbf{12.0\times}$ at 32K
tokens (cold prefill 15.4\,s vs SemBlend 1.3\,s at 32K);
(7)~hit rate never degrades as the donor pool grows---100\% hit rate
and $3.2$--$3.4\times$ speedup at 2K tokens across pool sizes 1--100
(100 unique donors, no LRU eviction within the 15\,GB CPU buffer),
and $10.5$--$10.7\times$ at 8K across pool sizes 1--16; the two
caching mechanisms provide complementary coverage: prefix caching
for exact repeats, SemBlend for semantic variants; and
(8)~across all 7 variation types spanning cosine similarity 1.0 (exact
copy) to 0.20 (topically diverse), PPL ratio stays at $1.000\pm0.001$
on \emph{both} Qwen2.5-7B-AWQ and LLaMA-3.1-8B-AWQ---SemBlend provides
a \emph{quality floor guarantee} independent of model architecture: any
query receiving a KV hit uses verified exact-match blocks (no quality
loss), while any query receiving a miss falls back to cold inference
(PPL ratio $\equiv 1.0$ by definition), with no intermediate degraded
state.

Across 33,160 WildChat conversation pairs, a closed-form operating
envelope $\mathrm{E}[\text{TTFT}] = P_h \cdot t_{\text{hit}} +
(1{-}P_h) \cdot t_{\text{cold}}$ (with $t_{\text{hit}}{=}303\,\text{ms}$,
$t_{\text{cold}}{=}3{,}428\,\text{ms}$ at 8K tokens) predicts measured
TTFT within 5\% across all hit fractions; at the observed 33.3\%
real-world hit rate, SemBlend delivers a $\mathbf{1.16}\times$ E[TTFT]
improvement with no model changes or retraining.
