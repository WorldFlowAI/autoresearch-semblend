...
For long prompts ($T > 2{,}000$ tokens), MiniLM's 512-token window
would see only the first ${\sim}12\%$ of a 16K document if naive
head-truncation were used. \semblend instead applies a
\emph{sliding window decode}: 40\% of tokens are drawn from the
beginning (instruction + document start), 30\% from the center, and
30\% from the end, for a 2,000-token sample covering the full
document. This preserves both system-instruction identity and
body-content similarity signals, and is applied symmetrically at
query time and donor registration, ensuring cosine similarity
reflects the full-document content.
