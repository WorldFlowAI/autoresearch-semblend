...
\paragraph{Indirect validation via REORDER variation sensitivity.}
Stronger behavioral evidence for RoPE correction comes from the variation
sensitivity experiment (\S\ref{sec:analysis}).
REORDER variations---where the same article sentences appear in a
different order---achieve 50\% hit rate at 8K tokens on CNN/DailyMail
(Qwen) and 62.5\% on LLaMA. When LMCache injects KV blocks from the
donor sequence, those blocks land at \emph{different absolute positions}
in the target prompt, activating the RoPE delta correction kernel.
The PPL ratio for these REORDER hits is $1.000 \pm 0.001$ on both models,
confirming that the correction exactly restores the intended positional
encoding. Without correction (identity position map), $K_\text{corrected} = K_\text{donor}$
would carry wrong RoPE phases, producing attention patterns inconsistent
with the target sequence and degrading output quality. The measured
PPL $\approx 1.0$ on reordered-content hits thus provides direct behavioral
evidence that FM1 is resolved.
