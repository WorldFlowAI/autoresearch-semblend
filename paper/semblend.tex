...
\noindent Miss overhead ($O_{\mathrm{miss}}{=}\text{Miss}{-}\text{Cold}$)
ranges from 5\,ms at 4K to 212\,ms at 16K; at steady-state store sizes
($\sim$500 donors) it grows to ${\approx}496$\,ms.
Let $P_h^* = O_{\mathrm{miss}} / (O_{\mathrm{miss}} + S_{\mathrm{hit}})$
denote the break-even hit rate, where $S_{\mathrm{hit}}{=}\text{Cold}{-}\text{Hit}$.
\semblend\ is net-positive at \textbf{all tested context lengths} for
virtually any nonzero hit rate ($P_h^*{<}5\%$ at 4--16K tokens).
