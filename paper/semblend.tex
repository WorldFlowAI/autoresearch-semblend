...
SemBlend provides a \emph{quality floor guarantee}: any query receiving
a KV hit uses verified exact-match blocks (no quality loss), while any
query receiving a miss falls back to cold inference (PPL ratio $\equiv
1.0$ by definition), with no intermediate degraded state.
