...
At $\geq$8K tokens, the blended TTFT equals cold TTFT only at $P_h{=}0$;
any nonzero hit rate strictly reduces latency. At $P_h{=}0.25$, blended
TTFT is $0.77\times$ cold (23\% reduction). At the production default
($\tau{=}0.60$, 62--100\% hit rate across datasets),
Eq.~\ref{eq:expected-ttft} predicts blended TTFT of
$0.36\times$--$0.09\times$ cold---matching the
$2.2$--$10.7\times$ range in Table~\ref{tab:cross-dataset}.
At 4K tokens, live break-even measurement ($n{=}500$ per condition)
shows that KV retrieval overhead exceeds prefill savings: hit TTFT
averages 1,359\,ms vs.\ cold 1,278\,ms ($+6.3\%$), confirming that
\semblend's benefits emerge at longer contexts where prefill
dominates inference latency (code generation at 4K shows only 4\%
overhead; see \S\ref{sec:codegen}).
