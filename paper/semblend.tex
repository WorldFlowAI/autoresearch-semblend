...
\item \textbf{Empirical evaluation} on two model architectures
  ...
  datasets, demonstrating $2.4\times$--$8.3\times$ TTFT speedup at
  2K--16K tokens with quality preserved (PPL ratio ${\leq}1.025$ on
...
and decreases only marginally to $9.3\times$ at 32 donors (as LRU
...
Qwen's 38--75\%, so more miss-runs inflate the mean PPL.
...
miss-run outliers (62--75\% hit rate), not KV injection degradation.
Hit rate is the primary predictor of mean PPL: ${\geq}88\%$ hit
$\Rightarrow$ PPL $\leq 1.065$ consistently across both models.}
\end{table}
...
We validate the 0.60 threshold empirically on CNN/DailyMail with Qwen2.5-7B-AWQ
($n{=}8$ per threshold). Lower thresholds improve hit rate at the cost of quality:
similarity $\geq 0.40$ achieves 75\% hit rate but PPL ratio 1.10, which exceeds
our 1.05 quality ceiling. Similarity $\geq 0.60$ achieves 62\% hit rate with PPL
ratio 1.022---the best quality/hit balance. Similarity $\geq 0.80$ drops to 38\%
hit rate, sacrificing throughput without further quality benefit.
The 0.60 default is therefore an empirically-calibrated operating point,
...
hardware---with quality preserved (PPL ratio ${\leq}1.025$ on high-hit
...
TTFT speedup with 75--100\% hit rates at 8K tokens, and LLaMA-3.1-8B
consistently achieves higher hit-only speedups ($2.7\times$--$3.8\times$)
than Qwen ($1.5\times$--$2.5\times$) due to its deeper architecture
