...
threshold (float32 precision ceiling).
...
Direct empirical validation for $\Delta {\neq} 0$ scenarios (sub-chunk
or sentence-level injection) requires future benchmarking.
Mathematical correctness of RoPE delta correction for $\Delta {\neq} 0$
is verified against ground-truth RoPE recomputation.
...
Correction becomes essential for sub-chunk or sentence-level injection
(planned), where $\Delta{\neq}0$; the Triton kernel correctness is verified
against ground-truth RoPE recomputation. Correction is enabled by default
as forward-compatible infrastructure.
