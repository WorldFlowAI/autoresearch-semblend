...
token overlap. Real-world datasets show $1.5\times$--$2.4\times$ at 8K
with 75--100\% hit rates. XSum and WikiHow achieve 100\% hit rate at 8K;
SAMSum and MultiNews reach 75--88\%; CNN/DailyMail the lowest at 50\%.
...
\item \textbf{MultiNews}: PPL ratio 1.005 at 4K---essentially cold
  baseline quality.
...
structured text (WikiHow) and multi-document summarization (MultiNews).
Dialogue datasets (SAMSum) show elevated mean PPL primarily due to
miss-runs, motivating improved hit-rate mechanisms for conversational
text as future work.
...
datasets. WikiHow achieves 100\% hit rate on both models; SAMSum and XSum
achieve 75--88\% and 75--100\% respectively, confirming structured text
benefits most from SemBlend. LLaMA's advantage is architecture-driven:
32 layers vs 28 increases cold prefill cost while SemBlend's KV transfer
...
models. LLaMA on XSum shows PPL ratios 1.009/1.011/1.065 (2K/4K/8K),
comparable to Qwen XSum. On CNN/DailyMail, LLaMA shows slightly higher
PPL (1.039--1.077) than Qwen (0.960--1.060), which tracks the lower
hit rates: LLaMA achieves 25--62\% hit on CNN vs Qwen's 38--75\%,
so more miss-runs inflate the mean PPL. On WikiHow (100\% hit both
models), both models achieve PPL $\leq 1.025$.

\begin{table}[ht]
\centering
\caption{\textbf{Quality across datasets and models.}
PPL ratio $\approx 1.0$ means no degradation; elevated values on
CNN/DM and SAMSum are driven by miss-runs (failed KV transfer),
not by KV injection quality.}
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize WikiHow and XSum (high hit rate) show PPL $\leq 1.025$.
SAMSum and CNN/DM (lower hit rates) show elevated PPL due to miss-run
outliers, not KV injection degradation.}
\end{table}

This confirms SemBlend's RoPE correction and partial attention mechanisms
generalize across GQA configurations (4 vs 8 KV heads). Hit rate is the
primary predictor of mean PPL: datasets with ${\geq}88\%$ hit rate
consistently achieve PPL ${\leq}1.025$.
...
WikiHow, SAMSum), SemBlend achieves $1.5\times$--$3.3\times$ TTFT
speedup with 75--100\% hit rates at 8K tokens, and LLaMA-3.1-8B
consistently achieves higher hit-only speedups ($2.7\times$--$3.3\times$)
than Qwen ($1.5\times$--$2.4\times$) due to its deeper architecture
