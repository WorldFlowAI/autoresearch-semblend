...
high-drift early/late layers, while the scaled tier uses CAGRA GPU-ANN
search at $\mathbf{1.37}$\,ms constant latency up to $1\text{M}$ donors.
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (exact.west) -- ++(-0.5,0) |- node[left,font=\scriptsize,pos=0.3] {miss} (local);
  \draw[arrow] (exact.east) -- ++(0.5,0) |- node[right,font=\scriptsize,pos=0.3,text=gray] {miss + async} (scaled);
  ...
  \draw[arrow, color=tier1color] (exact.south) -- ++(0,-0.3)
    node[right,font=\scriptsize,color=tier1color] {hit} -- ++(0,-5.7) -- (inject.north);
\end{tikzpicture}
...
\textbf{SemBlend Local}: in-process MiniLM-L6-v2
...
\textbf{(3) Architecture scales from local numpy to GPU ANN.}
The in-process numpy tier handles up to 10K donors at ${\leq}0.14$\,ms;
the cuVS brute-force tier covers up to 100K at ${\leq}0.71$\,ms; CAGRA
ANN achieves $\mathbf{1.37}$\,ms \emph{constant} at $N{=}1\text{M}$
donors---locality-first with no network hops on the critical path.
