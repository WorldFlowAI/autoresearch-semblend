...
impossible with RoPE-based models. Evaluated on two model architectures
(Qwen2.5-7B-AWQ, LLaMA-3.1-8B-AWQ) and five real-world NLP datasets on A10G,
SemBlend achieves $1.5\times$--$3.8\times$ hit-only TTFT speedup on real data,
$7.8\times$--$8.3\times$ at 16K tokens on synthetic data, and
$10.5\times$--$10.7\times$ combined with vLLM prefix caching---with
output quality preserved (PPL ratio ${\leq}1.065$ on high-hit datasets).
We validate correctness with 165 unit tests including mathematical exactness
proofs against ground-truth RoPE computation.
\end{abstract}
