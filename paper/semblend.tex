...
Extended evaluation at \texttt{max\_tokens=512} on XSum 8K synthetic
clusters (exact-match hits, $n{=}8$) shows non-collapsing runs (75\%
of trials) achieve PPL ratio $= 1.048$ and ROUGE-L ${\approx}0.85$--$1.0$,
confirming quality stability at realistic generation lengths. The
remaining 25\% trigger an EOS-collapse artifact (see \S\ref{sec:discussion}
Limitations); output-length monitoring or a \texttt{min\_tokens}
constraint detects and mitigates this in production.
...
\texttt{max\_tokens=256}. Extended evaluation at \texttt{max\_tokens=512}
on XSum 8K synthetic clusters reveals that approximately 25\% of
exact-match hits trigger an \textit{EOS-collapse} artifact---the
model generates near-zero output before emitting \texttt{</s>}
because the injected donor KV places the attention state at a
natural sentence boundary. Non-collapsing hits (75\% of runs)
achieve ROUGE-L ${\approx}0.85$--$1.0$ with PPL ratio 1.048
(vs.\ 1.025 at \texttt{max\_tokens=256}), indicating the quality
metric remains stable for genuine hits; EOS-collapse is detectable
only through output-length monitoring. A \texttt{min\_tokens}
constraint prevents empty output in production deployments
(implemented in our benchmark harness via the vLLM API), though
it inflates PPL for the forced tokens---quality should be
reported on non-collapsed hits only. Instruction-based prompting
(``Continue writing:'') is an alternative mitigation that avoids
forced-generation artifacts.
