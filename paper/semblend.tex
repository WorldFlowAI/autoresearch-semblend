...
retrieval-augmented generation workloads. KVLink~\cite{kvlink} precomputes
...
Concurrent work on KV cache physics~\cite{kvphysics} identifies
architecture-specific information flow patterns: LLaMA-family models
exhibit an ``inverted funnel'' (early-layer consolidation), while
Qwen-family shows a ``funnel'' (late-layer consolidation). Our
calibrated bathtub curve (\S\ref{sec:recomp}) implements this principle
directly: LLaMA parameters ($\sigma_e{=}0.45$, $\sigma_l{=}0.15$)
recompute early layers $[0,1,2]$ (9.4\%), while Qwen parameters
($\sigma_e{=}0.15$, $\sigma_l{=}0.35$) recompute late layers
$[26,27]$ (7.1\%), matching each architecture's consolidation pattern.
