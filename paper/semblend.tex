...
speedup at 2K--32K tokens. On datasets with ${\geq}88\%$ hit rate
...
by miss-run outliers, not KV injection. On real WildChat conversation
pairs at 33\% hit rate, \semblend delivers a $1.16\times$ mean TTFT
improvement with no model modifications.
\end{abstract}
...
of the token sequence before feeding it to MiniLM. This sampling is
applied symmetrically at query time and donor registration.
...
$n{=}5$ per length (short/mid-range); $n{=}8$ for 24K/32K (see $n$ column). In controlled single-donor scenarios with
...
at 8K tokens, speedup ranges $1.97$--$3.71\times$ on high-hit conditions
...
$2.2\times$ at 2K, $5.8\times$ at 8K, $8.3\times$ at 16K,
$9.0\times$ at 24K, and $\mathbf{{\sim}12\times}$ at 32K tokens
...
on content the prefix cache cannot match, and $3.0\times$--$3.4\times$
on cross-instruction queries where LMCache achieves no reuse at all.
...
is the PPL ratio: \textbf{11 of 12 PPL ratios are within 3\% of 1.0}
(range: 0.984--1.041; 11/12 within 3\%), confirming that the model's per-token confidence
...
\paragraph{Extended generation quality (256--512 tokens).}
...
across all lengths and both models (range: 0.984--1.041; 11/12 within 3\%), confirming
...
Extended evaluation at \texttt{max\_tokens=512} and \texttt{1024}
on XSum 8K synthetic clusters (exact-match hits, $n{=}8$) confirms
quality stability at realistic generation lengths:
at 512 tokens, PPL ratio $= 1.048$ and ROUGE-L ${\approx}0.85$--$1.0$;
at 1024 tokens, PPL ratio $= 0.997$ (better than cold) and ROUGE-L $= 0.677$.
The ROUGE-L decrease at longer generation is expected---early-token divergence
cascades over longer outputs---while PPL remains stable near 1.0,
confirming SemBlend quality at production generation lengths.
...
matches the Qwen preset ($\sigma_l{=}0.35 {>} \sigma_e{=}0.15$),
validating the direction of the architecture-specific calibration.
...
\noindent At 2K the pipeline overhead is \emph{zero}---embedding and vector
search complete within the prefill compute window. Miss overhead grows
...
LMCache stores KV in fixed-size chunks (default 256 tokens). Cross-instruction
reuse---where the same document is queried with different instruction prefixes---is
limited by chunk boundary alignment: an instruction prefix change of $k$ tokens shifts
all subsequent chunk boundaries, invalidating any chunk whose tokens straddle the
original boundary. Smaller chunks reduce this sensitivity.
...
speedup of $0.88\times$ reflects net SemBlend overhead on misses
(40\% of queries with misaligned instructions), consistent with the
short-context overhead observed in break-even analysis
...
text-level recovery). The correction adds ${\sim}2.7$\,s per request
for the $\mathrm{RoPE}(-\Delta)$ application across 28 layers on A10G,
confirming the Triton kernel operates correctly end-to-end under
realistic vLLM deployment with LMCache chunk-aligned KV injection.
...
To validate end-to-end TTFT gains on real user traffic, we select
long-context WildChat pairs ($\geq$6,000 characters; average 3,315
donor / 2,659 query tokens) and run live inference against
Qwen2.5-7B-AWQ. Of 30 pairs, 33.3\% receive a SemBlend hit
(cosine$\geq$0.60). Hit-only TTFT is \textbf{1.74$\times$ (p50)}
with a peak of $2.22\times$, compared to a cold baseline of 1,596\,ms.
Applying the operating envelope model (Eq.~\ref{eq:expected-ttft}) to
this workload ($P_h{=}0.333$, $\text{TTFT}_\text{hit}{=}917$\,ms,
$\text{TTFT}_\text{cold}{=}1596$\,ms) gives
$\mathbb{E}[\text{TTFT}] = 0.333 \times 917 + 0.667 \times 1596
= 1{,}370$\,ms, a $\mathbf{1.16\times}$ improvement over cold serving
even though two-thirds of requests miss.
These speedups are consistent with our controlled-dataset results
at the 2K--4K token range (Table~\ref{tab:results-kv}), confirming
that real-user conversation pairs activate SemBlend with equivalent
efficiency to purpose-built benchmark prompts.
...
LLaMA-3.1-8B @ 16K tokens (A10G) & 6{,}377 & per-length server restart \\
\bottomrule
\end{tabular}\\[2pt]
...
(P50 hit-only; Qwen2.5-7B-AWQ). On
real NLP datasets with mixed semantic variation types, speedup ranges
$1.97\times$--$3.78\times$ at 8K tokens on high-hit conditions
(Qwen: $1.97$--$2.37\times$; LLaMA: $2.66$--$3.78\times$ hit-only). SemBlend TTFT remains
...
a controlled 8-worker fleet (\S\ref{sec:scaled}), achieving $2.4$--$3.8\times$
...
conversations, multi-turn dialogue (\S\ref{sec:multiturn}), and code
generation (\S\ref{sec:codegen}). Larger-scale diverse production workloads
(customer support, search) are needed to more rigorously characterize
...
SemBlend achieves $\mathbf{2.2\times}$--$\mathbf{{\sim}12\times}$ TTFT
speedup across 2K--32K tokens on two model architectures, with TTFT
remaining approximately constant (${\sim}500$\,ms--$1.3$\,s)
regardless of prompt length. At 32K tokens cold prefill takes 15.4\,s;
SemBlend completes in 1.3\,s. On five real NLP datasets, hit-only
speedup ranges $1.97\times$--$3.78\times$ at 8K tokens.
