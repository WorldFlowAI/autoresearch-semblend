...
position encoding exactly, \textbf{attention pattern drift} from context
differences is addressed heuristically by the bathtub curve; a more
principled per-request calibration could further improve quality. The
bathtub presets are calibrated for Qwen2.5-7B and LLaMA-3.1-8B; GQA,
MoE, and very deep models ($>$80 layers) may require recalibration.
...
speedup on exact-match pairs; routing accuracy on diverse production
workloads will depend on the semantic overlap distribution of incoming
traffic. Empirical per-instance isolation is additionally confounded
by vLLM's internal RadixAttention prefix cache.
Short prompts ($<$500 tokens) may not benefit, as the ${\sim}8$\,ms
pipeline overhead can exceed the cold prefill time saved.
\textbf{Benchmark scope.} Current evaluations use NLP summarization
datasets (CNN/DM, XSum, WikiHow, SAMSum, MultiNews) and the WildChat
conversational corpus. These cover important cases but are not purpose-built
for semantic caching. Larger-scale, real-world datasets specifically
designed to capture the distribution of semantically related but
lexically distinct queries across diverse production workloads---code
generation, customer support, search, multi-turn dialogue---are needed
to more rigorously characterize the operational envelope of semantic
KV reuse systems.
