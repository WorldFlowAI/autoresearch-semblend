...
The blended TTFT equals cold TTFT only at $P_h{=}0$; any nonzero
hit rate strictly reduces latency. At $P_h{=}0.25$, blended TTFT
is $0.77\times$ cold (23\% reduction). At the production default
...
\centering
\small
\caption{\textbf{Chunk size ablation (offline alignment, $n{=}1000$).}
Cross-instruction reuse ratio at 8K tokens.
Smaller chunks recover more reuse because fewer boundaries are disrupted
by instruction-prefix shifts (${\sim}50$ tokens).}
\label{tab:chunk-size-ablation}
\begin{tabular}{rrr}
\toprule
\textbf{Chunk Size} & \textbf{8K Reuse} & \textbf{Relative} \\
\midrule
64   & 48.1\% & $2.5\times$ vs.\ 512 \\
128  & 41.7\% & $2.1\times$ \\
256  & 25.1\% & $1.3\times$ \\
512  & 19.4\% & $1.0\times$ (baseline) \\
\bottomrule
\end{tabular}
\end{table}

At 8K tokens, chunk size 64 achieves $2.5\times$ more cross-instruction
reuse than 512. However, smaller chunks increase metadata overhead (more
LMCache chunk entries per prompt) and reduce exact-match probability for
document-body chunks. The current default of 256 balances reuse opportunity
against LMCache storage efficiency.
...
\textbf{Benchmark scope.} Current evaluations use NLP summarization
datasets (CNN/DM, XSum, WikiHow, SAMSum, MultiNews), the WildChat
conversational corpus, and code generation workloads (\S\ref{sec:results}).
Multi-turn dialogue and larger-scale diverse production workloads (customer
support, search) are needed to more rigorously characterize the operational
envelope.
