...
Coordinates the full SemBlend Local pipeline with zero network hops:
  ...
  2. Donor store lookup (~2ms) — numpy cosine similarity search
  3. rapidfuzz alignment (<1ms) — token-level edit distance
  4. Bathtub curve layer deviations (<0.1ms) — predict recomputation needs
  5. Build PartialAttention plan — consumed by Triton kernels or LMCache

Total overhead: ~6ms (excluding KV load from CPU offload).
"""
...
def _order_invariant_text(text: str, max_chars: int = 1500) -> str:
    ...
    Limits to max_chars to keep embedding fast.
    """
    # Split into sentences/lines and sort
...
class SemBlendPipeline:
    ...
    def __init__(
        ...
        logger.info(
            ...
            "max_donors=%d, min_sim=%.2f, min_reuse=%.2f",
    ...
    def find_donor_candidates(
        ...
        Used by the connector to try each candidate's KV in LMCache until
        one is found. This avoids the problem where the best-scoring donor's
        KV has been evicted from LMCache.

        Shares the embedding computation with find_donor — the embedding
        is computed once and reused for all candidates.
        """
        timings = PipelineTimings()
        t_start = time.monotonic()
    ...
    def build_partial_attention_plan(
        ...
        a structured plan consumed by the PartialAttentionHook and Triton
        kernels.

        Args:
            pipeline_result: A successful PipelineResult (found=True).

        Returns:
            PartialAttentionPlan or None if plan building fails.
        """
        if not pipeline_result.found or not pipeline_result.slot_actions:
            return None

        try:
            from synapse_kv_connector.partial_attention import build_attention_plan
    ...
    def _detect_num_layers(self) -> int:
        """Detect number of transformer layers from model name."""
        if self._model_name:
