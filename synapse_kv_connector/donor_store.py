...
class DonorStore:
    ...
    def find_donor(
        ...
        Pipeline:
          1. Cosine similarity on all entries -> top-k (threshold >= min_similarity)
          2. rapidfuzz alignment on top-k -> best donor with reuse_ratio >= min_reuse_ratio
        ...
        # Step 2: Alignment on top-k candidates
        best_match: DonorMatch | None = None
        best_score = 0.0

        # Map indices back to donor entries
        ...
        for i, storage_idx in enumerate(candidate_indices):
            ...
            # Score: similarity * reuse_ratio
            score = float(similarities[i]) * alignment.reuse_ratio
            if score > best_score:
                best_score = score
    ...
    def find_donors(
        ...
        one whose KV is still in LMCache.
        """
        ...
        # Alignment + scoring with recency tiebreaker
        idx_to_id = {v: k for k, v in self._id_to_idx.items()}
        now = time.monotonic()
        ...
        for i, storage_idx in enumerate(candidate_indices):
            ...
            # Recency bonus: up to 0.01 for donors created in the last 60s
    ...
    def find_candidates_jaccard(
        ...
        Computes |A intersection B| / |A union B| on token ID sets instead
        of embedding cosine similarity. This is a baseline for the embedding
        ablation benchmark (Ablation B) to show that learned embeddings
        capture semantic similarity that token-level Jaccard misses
        (especially paraphrase variations).
        ...
        Returns:
            List of DonorMatch objects ranked by Jaccard score (descending),
            with similarity field set to the Jaccard score.
        """
